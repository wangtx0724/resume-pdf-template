...
def paragraph(c, page, grid_y, value, size=13, leading=24):
    for line in wrap(value, size):
        draw(c, page, GRID_MARGIN, grid_y, line, size)
        grid_y += leading
    return grid_y


def inline(c, page, grid_y, label, value):
    ...
    if not remainder:
        return grid_y + 24
    return paragraph(c, page, grid_y + 24, remainder)
...
def page_one(c):
    ...
    paragraph(c, 0, 240, "面向生产、设备和仓储等场景，梳理用户任务、业务规则和操作路径，将复杂需求转化为清晰可执行的产品方案。")
    draw(c, 0, GRID_MARGIN, 288, "产品体验与界面设计：", 13, bold=True)
    paragraph(c, 0, 312, "关注企业软件的信息层级、异常反馈和操作效率，能够完成原型、界面方案及关键交互细节设计。")
    draw(c, 0, GRID_MARGIN, 336, "设计规范与协作：", 13, bold=True)
    paragraph(c, 0, 360, "使用Figma完成组件复用、标注和评审协作，推动多模块界面风格统一并提高开发还原效率。")
    draw(c, 0, GRID_MARGIN, 384, "AI与低代码验证：", 13, bold=True)
    paragraph(c, 0, 408, "使用AI工具辅助需求拆解、方案表达和MVP验证，具备低代码配置及企业客户培训支持经验。")

    section(c, 0, "工作经历", 464)
    entry(c, 0, 506, "云帆工业软件（示例）有限公司", "UI/产品设计", "2020.03-至今", 310)
    inline(c, 0, 535, "主导产品体验设计：", "负责设备管理、生产协同和数据看板等B端模块的需求梳理、交互原型与界面方案设计。")
    inline(c, 0, 559, "设计规范与交付：", "建立表单、表格和状态反馈等常用组件规范，配合研发完成评审、联调与验收。")
    inline(c, 0, 607, "客户场景支持：", "整理现场业务流程和交付材料，支持方案演示、问题收集与迭代优化。")
    inline(c, 0, 631, "跨团队协同：", "与研发、测试和实施团队保持沟通，推动需求按计划落地并持续优化关键流程。")

    section(c, 0, "项目经历", 687)
    entry(c, 0, 729, "智能设备管理平台（示例项目）", "交互设计师&PM", "2024.04-2025.02", 348)
    draw(c, 0, GRID_MARGIN, 757, "内容：", 14, bold=True)
    inline(c, 0, 780, "项目背景：", "面向制造现场的设备资产与维保管理系统，覆盖台账、点检、维保、请购和异常预警业务。")
    inline(c, 0, 804, "核心要素：", "梳理设备从采购、领用、点检、维保到报废的流程，完成监控看板、审批和资产评估等模块设计。")
    draw(c, 0, GRID_MARGIN, 857, "业绩：", 14, bold=True)
    paragraph(c, 0, 880, "完成核心模块原型与界面方案，支持客户演示、研发联调和现场验证。")

    entry(c, 0, 927, "制造执行协同系统（示例项目）", "产品体验设计", "2023.03-2024.03", 305)
    draw(c, 0, GRID_MARGIN, 955, "内容：", 14, bold=True)
    inline(c, 0, 978, "项目背景：", "为生产计划、工单执行和异常反馈提供统一的任务协同与数据追溯入口。")
    inline(c, 0, 1002, "设计行动：", "围绕计划、执行、异常和复盘场景梳理信息架构，设计列表、详情、筛选和状态反馈机制。")
    draw(c, 0, GRID_MARGIN, 1079, "业绩：", 14, bold=True)
    paragraph(c, 0, 1102, "形成可复用的界面规范，支撑多个业务模块同步开发和交付。")
...
def page_two(c):
    ...
    inline(c, 1, 1214, "项目背景：", "面向企业内部知识问答和应用配置需求，提供内容管理、流程配置、权限设置及客户培训支持。")
    inline(c, 1, 1238, "设计行动：", "参与需求访谈和原型设计，梳理知识录入、应用发布、权限配置与使用反馈流程。")
    draw(c, 1, GRID_MARGIN, 1291, "业绩：", 14, bold=True)
    paragraph(c, 1, 1314, "支持多个方案演示与内部试点，沉淀常见场景的配置说明和交付素材。")
