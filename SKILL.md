...
5. Use the full official Alibaba PuHuiTi 3.0 cache as the default typeface: `assets/fonts/AlibabaPuHuiTi-3`. If it is absent, run `python3 scripts/install_alibaba_puhuiti.py` before authoring. The reference PDF embeds character subsets only; never reuse those subsets to generate new text because missing glyphs can occur. Do not silently substitute a system font or claim exact typeface fidelity without this complete font.
